...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        self.encoder_norm_1 = LayerNorm(config.hidden_size, eps = 1e-6)
    ...
    def forward(self, hidden_states):
        ...
        for i,layer_block in enumerate(self.layer):
            if i%3 == 0 and i != 0:
                non_local.append(self.encoder_norm_1(hidden_states))
        ...
        #for i in range(len(non_local)):
            #non_local_1.append(non_local[len(non_local)-1-i]) 
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None,non_local=None):
        x = self.up(x)
        if skip is not None and non_local is not None:
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        head_channels_1 = 256
        head_channels_2 = 128
        self.up_1 = nn.ConvTranspose2d(head_channels_1,head_channels_1,4,4,0)
        self.up_2 = nn.ConvTranspose2d(head_channels_2,head_channels_2,8,8,0)
        ...
        self.up = nn.ConvTranspose2d(head_channels,head_channels,2,2,0)
    ...
    def forward(self, hidden_states, features=None,NON_LOCAL=None):
        ...
        NON_LOCAL[2] = self.conv_more_1(NON_LOCAL[2])
        NON_LOCAL[2] = self.up(NON_LOCAL[2])
        #NON_LOCAL[2] = self.bn_1(NON_LOCAL[2])
        ...
        NON_LOCAL[1] = self.up_1(NON_LOCAL[1])
        #NON_LOCAL[1] = self.bn_2(NON_LOCAL[1])
        ...
        NON_LOCAL[0] = self.conv_more_3(NON_LOCAL[0])
        NON_LOCAL[0] = self.up_2(NON_LOCAL[0])
        #NON_LOCAL[0] = self.bn_3(NON_LOCAL[0])

        NON_LOCAL.append(None)
        ...
        for i, decoder_block in enumerate(self.blocks):
            if features is not None:
                ...
                skip = None
            x = decoder_block(x, skip=skip,non_local=NON_LOCAL[2-i])
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        x, attn_weights, features,non_local = self.transformer(x)  # (B, n_patch, hidden)
        x = self.decoder(x, features,non_local)
